...
- (instancetype)init {
    if (self = [super init]) {
        self.logNameManager = [[YUILogNameManager alloc] init];
    }
    return self;
}

- (void)printLogWithFile:(const char *)file line:(int)line func:(const char *)func logItem:(YUILogItem *)logItem {
    // 禁用了某个 name 则直接退出
    ...
    if ([self.delegate respondsToSelector:@selector(printYUILogWithFile:line:func:logItem:defaultString:)]) {
        [self.delegate printYUILogWithFile:fileString line:line func:funcString logItem:logItem defaultString:defaultString];
    } else {
